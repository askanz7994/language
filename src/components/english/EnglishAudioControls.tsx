...
const EnglishAudioControls = ({
  ...
  return <div className="flex gap-4 justify-center mb-6">
      {!isReading ? <Button onClick={readText} disabled={isRecording} className="glow-button flex items-center gap-2 text-xs">
          <Volume2 className="h-4 w-4" />
          Listen Text
        </Button> : <Button onClick={stopReading} variant="destructive" className="flex items-center gap-2">
          <Square className="h-4 w-4" />
          Stop Listening
        </Button>}

      {!isRecording ? <Button onClick={onStartRecording} disabled={isReading} className="glow-button flex items-center gap-2 text-xs">
          <Mic className="h-4 w-4" />
          Start Reading
        </Button> : <Button onClick={onStopRecording} variant="destructive" className="flex items-center gap-2">
          <Square className="h-4 w-4" />
          Stop Reading
        </Button>}
    </div>;
